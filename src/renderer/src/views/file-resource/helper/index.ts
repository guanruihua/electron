...
export const getFileType = (item: FileNode) => {
  ...
  const match = (v)=> name.toLowerCase().endsWith(v)
  ...
  if (['.ts'].some((v) => match(v))) return 'ts'
  if (['.js', '.cjs', '.mjs'].some((v) => match(v))) return 'js'
  if (['.json', '.jsonc'].some((v) => match(v))) return 'json'
  if (['.html', 'htm'].some((v) => match(v))) return 'html'
  if (['.css'].some((v) => match(v))) return 'css'
  if (['.lnk'].some((v) => match(v))) return 'lnk'
  if (['.exe'].some((v) => match(v))) return 'exe'
  return 'file'
}
